...
#include <math.h>
#include <Python.h>
double Q2(double x, double *q, int **index_matrix, double hh, int s);
double NQ(double z, int i, double hh);
double M2(double x, double *q, int **index_matrix, double hh, int s);
double NM(double z, int i, double hh);
double Ntheta(double z, int i, double hh);
double theta2(double x, double *q, int **index_matrix, double hh, int s);
double Nw(double z, int i, double hh);
double w2(double x, double *q, int **index_matrix, double hh, int s);
int fill_matrix(double **A, double hh, double v, double EJ, int s);
...
int main(void)
{
  ...
  hh = hh/s;
  fill_matrix(A, hh, k, EJ, s);
  qh = q * hh;
  B[4*s+1] =  M_1;
  B[12*s] = P;
  B[16*s] = 0.5 * qh;
  B[16*s+1] = 1.0/12.0 * qh * hh;
  for(i = 0; i < 9*s-1; ++i) B[16*s + 2 + 2 * i] = qh;
  B[22*s] = 0.0;
  B[32*s] = 0.0;
  B[34*s] = 0.5 * qh;
  B[34*s+1] = 1.0/12.0 * qh * hh * (-1.0);
  LUdecompose(&A, &L, &U, N);
  LUsolve(&L, &U, B, X, N);
  index_matrix = (int **) calloc(20*s, sizeof(int *));
  for(i = 0; i < 20*s; i++)
  {
  ...
  for(t = 0.0; t <= l; t = t + 0.01) fprintf(output, "%e %.15e\n", t, w2(t, X, index_matrix, hh, s));
  fclose(output);
  output = fopen("theta2_data.txt", "w");
  for(t = 0.0; t <= l; t = t + 0.01) fprintf(output, "%e %.15e\n", t, theta2(t, X, index_matrix, hh, s));
  fclose(output);
  output = fopen("M2_data.txt", "w");
  for(t = 0.0; t <= l; t = t + 0.01) fprintf(output, "%e %.15e\n", t, EJ * M2(t, X, index_matrix, hh, s));
  fclose(output);
  output = fopen("Q2_data.txt", "w");
  for(t = 0.0; t <= l; t = t + 0.01) fprintf(output, "%e %.15e\n", t, EJ * Q2(t, X, index_matrix, hh, s));
  fclose(output);
  Py_Initialize();
  test = _Py_fopen("test2.py", "r");
  PyRun_SimpleFile(test, "test2.py");
  Py_Finalize();
  return 0;
}
double Nw(double z, int i, double hh)
...
double Q2(double x, double *q, int **index_matrix, double hh, int s)
{
    ...
    if(x - 20*hh*s == 0.0)
    {
        for(j = 0; j < 4; j++) sum = sum + q[index_matrix[20*s - 1][j]] * NQ(hh, j, hh);
        return sum;
    }
    while (!((a <= x) && (x < a + hh)))
    {
...
double M2(double x, double *q, int **index_matrix, double hh, int s)
{
    ...
    if(x - 20*hh*s == 0.0)
    {
        for(j = 0; j < 4; j++) sum = sum + q[index_matrix[20*s - 1][j]] * NM(hh, j, hh);
        return sum;
    }
    while (!((a <= x) && (x < a + hh)))
    {
...
double theta2(double x, double *q, int **index_matrix, double hh, int s)
{
    ...
    int i = 0, j;
    if(x == 0.0) return q[1];
    if(x - 20*hh*s == 0.0) return q[40*s + 1];
    while (!((a <= x) && (x < a + hh)))
    {
        a = a + hh;
        i = i + 1;
...
double w2(double x, double *q, int **index_matrix, double hh, int s)
{
    ...
    int i = 0, j;
    if(x == 0.0) return q[0];
    if(x - 20*hh*s == 0.0) return q[40*s];
    while (!((a <= x) && (x < a + hh)))
    {
        a = a + hh;
        i = i + 1;
...
int fill_matrix(double **A, double hh, double v, double EJ, int s)
{
    ...
    for(i = 0; i < 4; ++i)
  {
    ...
    //getchar();
    A[40*s][40*s] = A[40*s][40*s] + v;
    for(j = 0; j < 2*(20*s+1); j++) {A[0][j] = 0.0; A[j][0] = 0.0;}
    A[0][0] = 1.0;
    for(j = 0; j < 2*(20*s+1); j++) {A[1][j] = 0.0; A[j][1] = 0.0;}
    A[1][1] = 1.0;
    for(j = 0; j < 2*(20*s+1); j++) {A[8*s][j] = 0.0; A[j][8*s] = 0.0;}
    A[8*s][8*s] = 1.0;
    for(j = 0; j < 2*(20*s+1); j++) {A[22*s][j] = 0.0; A[j][22*s] = 0.0;}
    A[22*s][22*s] = 1.0;
    for(j = 0; j < 2*(20*s+1); j++) {A[32*s][j] = 0.0; A[j][32*s] = 0.0;}
    A[32*s][32*s] = 1.0;
    //for(k = 0; k < 42; ++k)
